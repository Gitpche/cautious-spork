...
namespace MyMod.Content.NPCs
{
    public class VoidOverseer : ModNPC 
    {
        ...
        public override void ModifyNPCLoot(NPCLoot npcLoot) {
            // 1. Фрагмент пустоты — главный аксессуар (шанс 100%)
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<VoidFragment>(), 1));

            // 2. Блоки Voidstone Crack — от 50 до 100 штук
            // Выпадают гарантированно при каждой смерти босса
            npcLoot.Add(ItemDropRule.Common(ModContent.ItemType<VoidstoneCrackItem>(), 1, 50, 100));

            // 3. Золотые монеты — 50 штук (как указано в цене босса)
            npcLoot.Add(ItemDropRule.Common(ItemID.GoldCoin, 1, 50, 50));
        }
    }
}